...
import { Text, FAB, Portal, Modal, TextInput, Button, Card, Snackbar, ActivityIndicator, Chip, Menu } from "react-native-paper";
...
export default function ManageAgents() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [agentsRes, centersRes] = await Promise.all([API.get("/auth/agents"), API.get("/centers")]);
      setAgents(agentsRes.data);
      setCenters(centersRes.data);
    } catch { setSnack("Failed to load agents or centers"); }
  ...
  const handleAdd = async () => {
    if (!name || !username || !password || !percentage || !selectedCenter) { setSnack("Fill all fields"); return; }
    setSaving(true);
    try {
      await API.post("/auth/register", { 
        ...
        password, 
        role: "agent", 
        agentPercentage: parseFloat(percentage),
        centerId: selectedCenter._id
      });
      setVisible(false); setName(""); setUsername(""); setPassword(""); setPercentage(""); setSelectedCenter(null);
      loadData(); setSnack("Agent added!");
    } catch (e) { setSnack(e.response?.data?.message || "Error adding agent"); }
    ...
    <View style={styles.container}>
            ...
            <Card style={styles.card}>
              <Card.Content>
                <View style={styles.row}>
                  ...
                  <Chip textStyle={{ color: balanceColor(item.balance), fontWeight: "700" }}
      ...
      <Portal>
        <Modal visible={visible} onDismiss={() => setVisible(false)} contentContainerStyle={styles.modal}>
          ...
          <TextInput label="Commission %" value={percentage} onChangeText={setPercentage} keyboardType="numeric" mode="outlined" style={styles.input} />
          <Menu
            visible={menuVisible}
          ...
          <Button mode="contained" onPress={handleAdd} loading={saving} disabled={saving} style={{ marginTop: 12 }}>Add Agent</Button>
          <Button mode="text" onPress={() => setVisible(false)}>Cancel</Button>
        </Modal>
      </Portal>

      <FAB icon="plus" style={styles.fab} onPress={() => setVisible(true)} label="Add Agent" />
...
const styles = StyleSheet.create({
  ...
  centerChip: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#e3f2fd", paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12, marginTop: 4, alignSelf: "flex-start" },
